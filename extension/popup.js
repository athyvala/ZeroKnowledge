    ...
    sessions.forEach((session, index) => {
      ...
      if (session.expires_at) {
        ...
        if (remainingMs > 0) {
          ...
          if (remainingHours > 0) {
            expirationText = `<br>Expires in: ${remainingHours}h ${remainingMins % 60}m`;
          } else {
            expirationText = `<br>Expires in: ${remainingMins}m`;
          }
        } else {
          expirationText = '<br><span style="color: #dc3545;">Expired</span>';
        }
      }
        ...
        sessionItem.innerHTML = `
          <div class="session-info">
            ...
            <div class="session-meta">
              ...
              Date: ${session.created_at ? new Date(session.created_at).toLocaleDateString() : 'Unknown date'}${expirationText}
            </div>
          </div>
  ...
  window.loadSharedSession = async (sessionId) => {
    try {
      ...
      if (successCount > 0) {
        ...
        // Set up auto-logout for shared session if it has expiration info
        if (sessionData.expiresAt) {
          const expirationTime = new Date(sessionData.expiresAt).getTime();
          const now = Date.now();
          
          if (expirationTime > now) {
            // Store session info for auto-logout
            const sessionInfo = {
              domain: sessionData.domain,
              cookies: sessionData.cookies.filter((_, index) => index < successCount), // Only successful cookies
            ...
            const minutesUntilLogout = Math.ceil((expirationTime - now) / (60 * 1000));
            if (minutesUntilLogout > 0) {
              try {
                ...
                console.log(`Auto-logout alarm set for ${minutesUntilLogout} minutes for shared session`);
                
                // Show active session indicator
                checkActiveSession();
              } catch (alarmError) {
                console.error('Error setting auto-logout alarm:', alarmError);
              }
            }
            
            const remainingMins = Math.floor((expirationTime - now) / 60000);
            showStatus(`Session will auto-logout in ${remainingMins} minutes`, 'info');
          } else {
            showStatus('Warning: This shared session has already expired', 'warning');
          }
        }
